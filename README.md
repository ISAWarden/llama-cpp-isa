...
| `turboquant_metal` | Metal turbo2/3/4 KV writes, turbo4 attention and native WHT; Mac validation pending; requires `turboquant` | Build with Metal, use `-fa on -ctk turbo4_0 -ctv turbo4_0` |
...
`--spec-type emerald-mtp` requires `--spec-emerald-mtp-file PATH`; a missing path is rejected during argument parsing, before model loading or context allocation. Regular drafting is the default: `--spec-emerald-mtp-n-min` and `--spec-emerald-mtp-n-max` default to 8 and 16, with `1 <= n-min <= n-max <= 1024`. It matches exactly `n-match` context tokens, follows frequency winners up to `n-max`, and discards proposals shorter than `n-min`. Use `--spec-emerald-mtp-n-min 3 --spec-emerald-mtp-n-max 16` for a 3–16-token range.

`--spec-emerald-mtp-n-auto` opts into automatic drafting and cannot be combined with either manual length flag, including explicitly supplied defaults. Conflicts are rejected before model loading, regardless of argument order. In auto mode, each step tries suffixes from one token up to the maximum context depth and uses the shortest suffix with exactly one historically observed continuation. A single observation suffices. Ambiguous suffixes require a longer context; if none qualifies, drafting stops and retains any partial proposal, including one token. The internal draft ceiling is 16 tokens. The maximum context depth defaults to 24 tokens (`--n-match` in the importer, `--spec-emerald-mtp-n-match` in the server), supports 1–1024, and is inferred from an existing database unless explicitly supplied; an explicit value must agree. Upstream context, batch, and remaining-output limits still apply. `--spec-type emerald-mtp,ngram-mod` tries EmeraldMTP first and falls through on insufficient matches. Synthetic acceptance options and `tokenizer.*` metadata overrides are rejected.

All server slots share one database. A reverse token trie stores permanent continuation witnesses for every observed suffix up to the maximum depth. Exact parent/token keys resolve hash collisions. A second continuation permanently marks that suffix ambiguous, including after frequency eviction and restart. The existing context graph retains imported and learned frequency statistics, but its frequency winners do not decide automatic drafts. For example, separately tokenized `Position → Manager` can produce a one-token proposal; observing `Position → Other` disables that guess from `Position` alone. Token IDs, including whitespace variants, determine matches; the target verifies every proposed token. Text prompts are observed once when ready for generation; committed output tokens, including the final token, are learned as the server processes them. Rejected drafts and checkpoint/decode replay do not update the database. Slot release clears sequence tracking. Learning is disabled for multimodal prompts. Tokens that trigger a stop condition are included; later tokens in the verified batch are excluded.

The little-endian `EGRAPH02` database stores aligned sections for packed token ranges, context nodes, continuation counts, a persisted tagged hash index, and permanent continuation witnesses. Incompatible formats are rejected with a rebuild message. Context nodes cache the winning token and successor ID. It retains 64-bit offsets/counts and full tokenizer GGUF metadata as a collision-free compatibility identity, including vocabulary, merges, special-token settings, and other `tokenizer.*` fields. See the [context graph format](docs/emerald_mtp.md) for the layout and lookup rules. Checksummed append transactions preserve learned observations between snapshots. Recovery discards an incomplete final journal transaction; malformed or incompatible databases are rejected. An OS writer lock on the adjacent `.lock` file remains held throughout server/importer lifetime. Stop the server before appending with the importer.

The server worker flushes at one-second intervals and drains at orderly shutdown. An abrupt exit can lose observations not yet flushed. A persistence failure is reported and disables learning and automatic drafting. A small adjacent `.incomplete` marker is written when possible so restart also disables drafting; inference continues through ordinary target sampling or another configured speculator. Compaction atomically replaces the database and temporarily requires space for a second copy. Journal writes, snapshot packing, checksumming, disk sync, and snapshot replacement run outside the database mutex. Snapshot generation copying, learning, and retention still take that mutex and can delay lookups; compaction needs extra RAM for the captured graph and serialization buffers.

`--max-size-gb` (importer) and `--spec-emerald-mtp-max-size-gb` (server) limit compacted file contents, using decimal GB; `0` means unlimited. Retention removes the least recently observed learned entries first and preserves imported counts. Imports whose protected contents exceed the limit fail. Permanent continuation witnesses are protected along with imported counts. If new witnesses cannot fit, the database records incomplete history and disables learning and automatic drafting; rebuild at a new path to resume. Reopening with a limit below the protected contents fails. Compaction bounds journal growth without forcing a snapshot on every flush merely because a limit is configured; the limit does not bound RAM or temporary disk usage.
